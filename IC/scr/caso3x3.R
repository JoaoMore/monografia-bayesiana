...
# Caso 1 - Probabilidades próximas de 0.33 ---------------------------------
...
set.seed(1)
markov3x3.1 <- convergencia.markov(n = N, 
                              markov.chain = caso1, 
...
caso1.3x3.stan.beta <- stan(file = 'stan files/beta.stan', 
                   data = data.stan, chains = 4, seed = 1, iter = N*2)


df1 <- tibble(caso1.3x3.stan.beta %>% as.data.frame()) %>% 
...
caso1.3x3.stan.kuma <- stan(file = 'stan files/kuma.stan', 
                   data = data.stan, chains = 1, seed = 1, iter = N*2)


df1 <- tibble(caso1.3x3.stan.kuma %>% as.data.frame()) %>% 
  mutate(pm = cumsum(p)/1:N)
...
set.seed(1)
markov3x3.2 <- convergencia.markov(n = N, 
                              markov.chain = caso2, 
                              cell = c(1,1), #célula de interesse a se estimar da matriz de transição
                              initial.state = 'a')
...
caso2.3x3.stan.beta <- stan(file = 'stan files/beta.stan', 
                        data = data.stan, chains = 4, seed = 1, iter = N*2)


df1 <- tibble(caso2.3x3.stan.beta %>% as.data.frame()) %>% 
  mutate(pm = cumsum(p)/1:N)
...
caso2.3x3.stan.kuma <- stan(file = 'stan files/kuma.stan', 
                   data = data.stan, chains = 1, seed = 1, iter = N*2)


df1 <- tibble(caso2.3x3.stan.kuma %>% as.data.frame()) %>% 
  mutate(pm = cumsum(p)/1:N)
...
# Caso 3 - Probabilidades extremas nas três linhas ---------------------------------
...
set.seed(1)
markov3x3.3 <- convergencia.markov(n = N, 
                              markov.chain = caso3, 
                              cell = c(1,1), #célula de interesse a se estimar da matriz de transição
                              initial.state = 'a')
...
caso3.3x3.stan.beta <- stan(file = 'stan files/beta.stan', 
                        data = data.stan, chains = 4, seed = 1, iter = N*2)


df1 <- tibble(caso3.3x3.stan.beta %>% as.data.frame()) %>% 
  mutate(pm = cumsum(p)/1:N)
...
caso3.3x3.stan.kuma <- stan(file = 'stan files/kuma.stan', 
                   data = data.stan, chains = 1, seed = 1, iter = N*2)


df1 <- tibble(caso3.3x3.stan.kuma %>% as.data.frame()) %>% 
  mutate(pm = cumsum(p)/1:N)
